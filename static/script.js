...
const prospectsList  = $('prospects-list');
const platformSelect = $('platform-select');

// === Search ===
async function search() {
  const keyword = keywordInput.value.trim();
  const platforms = Array.from(platformSelect.selectedOptions).map(o => o.value).join(',');
  ...
  try {
    const res = await fetch(`${API_BASE}/search?keyword=${encodeURIComponent(keyword)}&platforms=${platforms}&page=${page}&per_page=${perPage}`);
  ...
  resultsList.innerHTML = searchResults.map((r, i) => `
    <div class="border rounded-lg p-4 hover:shadow-sm">
      <div class="flex justify-between">
        <span class="text-xs font-semibold px-2 py-1 bg-blue-100 text-blue-800 rounded">${r.platform}</span>
        <span class="text-xs px-2 py-1 rounded ${
          r.lead_score?.includes("Hot") 
            ? "bg-red-100 text-red-800" 
            : "bg-yellow-100 text-yellow-800"
        }">${r.lead_score || "New"}</span>
      </div>
      <h3 class="font-medium mt-1">
        <a href="${r.url}" target="_blank" class="hover:underline text-primary">${r.title || "Untitled"}</a>
      </h3>
      <p class="text-sm text-gray-600 mt-1">${r.snippet || ""}</p>
      <button onclick="useAsContext(${i})"
